...
from speechbrain.lobes.models.BESTRQ import brq_mask_collate_fn # from mask 
...
def compute_benchmark(batch, hparams, device="cuda"):
    ...
    # Move all modules in the hparams["modules"] dict to CUDA
...
def main():
    ...
    with open(hparams_file) as fin:
        ...
        print(f"starting experiment name: {hparams['expe_name']}")
    ...
    with torch.cuda.amp.autocast():
        ...
        save_results = []
        save_runs = []
        sim_time_list = [2]+[k for k in range(10, EVAL_MAX_DURATION, 10)]
        for sim_test_time in sim_time_list:
            avg_duration = 0.0
            std_duration = 0.0
    ...
    import os
    os.makedirs(os.path.join("./pom/eval/memory_results", hparams["expe_name"]), exist_ok=True)
    save_file = os.path.join("./pom/eval/memory_results", hparams["expe_name"], "memory_test.csv")
    ...
    save_file = os.path.join("./pom/eval/memory_results", hparams["expe_name"], "memory_runs.csv")
    with open(save_file, "w") as f:
