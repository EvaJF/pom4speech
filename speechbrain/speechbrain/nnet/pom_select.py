...
class PoM(nn.Module):
    ...
    """
    def __init__(self, dim, degree, expand, num_heads=1, bias=True, select=False):
        super().__init__()
        self.dim = dim
        self.order = degree
        ...
        self.select = select
        print("SELECT option in PoM: ", self.select)
        print(f"Degree {degree}, Expand {expand}")
        if select : # keep only the highest order
            self.po_proj = nn.Linear(dim, 1 * expand * dim, bias=bias) # TODO: to list / num_heads
            self.se_proj = nn.Linear(dim, 1 * expand * dim, bias=bias) # idem
            self.ag_proj = nn.Linear(1 * expand * dim, dim, bias=bias) # idem
        else : 
            self.po_proj = nn.Linear(dim, degree * expand * dim, bias=bias) # TODO: to list / num_heads
            self.se_proj = nn.Linear(dim, degree * expand * dim, bias=bias) # idem
            self.ag_proj = nn.Linear(degree * expand * dim, dim, bias=bias) # idem
    ...
    def forward(self, xq, xc=None, mask=None):
        ...
        s = self.se_proj(xq)
        h = self.po_proj(xc)
        sh = self.pom(s, h, self.order, mask, select=self.select)

        # aggregation
        return self.ag_proj(sh)
    
def pom(xq: torch.Tensor, xc: torch.Tensor, k: int, mask=None, select=False):
    """
    pom function
    ...
    This function implements the polynomial mixer operation.
            ...
            mask (torch.Tensor, optional): The mask tensor for attention.
    """
    h = polynomial_aggregation_(xc, k, mask, select=select)
...
def polynomial_aggregation_(x: torch.Tensor, k: int, mask=None, select=False):
    if select : 
        h = F.relu(x)
        for i in range(1, k):
            h = h * F.relu(x)
