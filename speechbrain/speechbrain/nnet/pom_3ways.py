...
class PolynomialMixer(nn.Module):
    ...
    def forward(
        ...
        TODO: what about attn_mask and pos_embs ?
...
class PoM(nn.Module):
    ...
    def __init__(self, dim, degree, expand, num_heads=1, bias=True):
        ...
        self.po_proj_1 = nn.Linear(dim//3, degree * expand * (dim//3), bias=bias)
        self.po_proj_2 = nn.Linear(2*dim//3 - dim//3, degree * expand * (2*dim//3 - dim//3), bias=bias)
        self.po_proj_3 = nn.Linear(dim-2*dim//3, degree * expand * (dim - 2*dim//3), bias=bias)
        self.se_proj_1 = nn.Linear(dim//3, degree * expand * (dim//3), bias=bias)
        self.se_proj_2 = nn.Linear(2*dim//3 - dim//3, degree * expand * (2*dim//3 - dim//3), bias=bias)
        self.se_proj_3 = nn.Linear(dim-2*dim//3, degree * expand * (dim - 2*dim//3), bias=bias)
    ...
    def forward(self, xq, xc=None, mask=None):
        ...
        # TODO if mask is not None

        # 3-way version
        xq_1 = xq[:, :, :self.dim//3] # slice features 1 : dim//3
        xq_2 = xq[:, :, self.dim//3:2*self.dim//3]
        xq_3 = xq[:, :, 2*self.dim//3:]
        xc_1 = xc[:, :, :self.dim//3] # slice features 1 : dim//3
        xc_2 = xc[:, :, self.dim//3:2*self.dim//3] 
        xc_3 = xc[:, :, 2*self.dim//3:]
        s_1 = self.se_proj_1(xq_1)
        s_2 = self.se_proj_2(xq_2)
        s_3 = self.se_proj_3(xq_3)
        h_1 = self.po_proj_1(xc_1) 
        h_2 = self.po_proj_2(xc_2)
        h_3 = self.po_proj_3(xc_3)
        sh_1 = self.pom(s_1, h_1, self.order, mask)
        sh_2 = self.pom(s_2, h_2, self.order, mask)
        sh_3 = self.pom(s_3, h_3, self.order, mask)
        sh = torch.cat([sh_1, sh_2, sh_3], dim=-1)
